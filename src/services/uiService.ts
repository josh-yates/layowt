...
export class UIService {
    ...
    public getContainerGridStyles(update: any): string {
        return `
                grid-template-columns: repeat(${this._gridService.getGridColumns()}, 1fr);
    ...
    public getPaneGridStyles(pane: TreeNode, update: any): string {
        return `
            grid-column: ${this._gridService.getIndex(pane, SplitType.Vertical) + 1} / span ${this._gridService.getSpan(pane, SplitType.Vertical)};
